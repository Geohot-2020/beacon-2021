...
/***********notes*************/
 /*
  * 1.记得加<stdio.h>，才能用sprintf
  * 2.uint8和uint16都是字符型，但能当数字用
  * 3.Openmv数据格式为：!000 !111 !222
  * 4.串口中断接收数据： UART0_RX_IRQHandler(void);
  */
/****************************/
#include <LQ_UART.h>
#include <LQ_GPIO.h>
...
//    UART_PutChar(UART0, UART_GetChar(UART0));
    ...
    if(a_cnt[0]==(uint8)'!')
    {
        a_cnt[cnt]=a[0];
        cnt++;
        if(cnt==4)      //结束标志，执行数据处理和存储
        {
        //        TFTSPI_P8X16Str(0, 7, (uint8)a_cnt[2], u16RED, u16BLUE);
//            UART_PutChar(UART0,a_cnt[3]);
            openmv_data = (a_cnt[1]-48)*100 + (a_cnt[2]-48)*10 + a_cnt[3]-48;
            char txt[32];

            sprintf(txt, "openmv: %03d", openmv_data);
...
//    char txt[40];
//    sprintf(txt, "variate:%d", a_cnt[2]);

//    TFTSPI_P8X16Str(0, 7, "1122", u16RED, u16BLUE);
...
/****************************************************************************
 * 函数名称：void UART_Openmv()
 * 功能说明：openmv端数据的接收并存入数组中
 * 参数说明：
 * 函数返回：
 * 修改时间：2021年10月20日
 * 备注：
